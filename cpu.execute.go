...
var cpuhandlers = map[instruction]InstructionHandler{
	...
	OpStoreMemA:                storeA,
	OpLoadMemA:                 loadA,
	OpLoadIndirectAHLIncrement: loadAInc,
	OpLoadIndirectAHLDecrement: loadADec,
	OpLoadIndirectHLAIncrement: storeAInc,
	OpLoadIndirectHLADecrement: storeADec,
	...
	OpAddDirectAANoCarry:       add8(RegA, false),
	OpAddDirectABNoCarry:       add8(RegB, false),
	OpAddDirectACNoCarry:       add8(RegC, false),
	OpAddDirectADNoCarry:       add8(RegD, false),
	OpAddDirectAENoCarry:       add8(RegE, false),
	OpAddDirectAHNoCarry:       add8(RegH, false),
	OpAddDirectALNoCarry:       add8(RegL, false),
	OpAddIndirectAHLNoCarry:    add8(RegHLInd, false),
	OpAddImmediateANoCarry:     addi(false),
	OpAddDirectAACarry:         add8(RegA, true),
	OpAddDirectABCarry:         add8(RegB, true),
	OpAddDirectACCarry:         add8(RegC, true),
	OpAddDirectADCarry:         add8(RegD, true),
	OpAddDirectAECarry:         add8(RegE, true),
	OpAddDirectAHCarry:         add8(RegH, true),
	OpAddDirectALCarry:         add8(RegL, true),
	OpAddIndirectAHLCarry:      add8(RegHLInd, true),
	OpAddImmediateACarry:       addi(true),
	OpSubDirectAANoCarry:       sub8(RegA, false),
	OpSubDirectABNoCarry:       sub8(RegB, false),
	OpSubDirectACNoCarry:       sub8(RegC, false),
	OpSubDirectADNoCarry:       sub8(RegD, false),
	OpSubDirectAENoCarry:       sub8(RegE, false),
	OpSubDirectAHNoCarry:       sub8(RegH, false),
	OpSubDirectALNoCarry:       sub8(RegL, false),
	OpSubIndirectAHLNoCarry:    sub8(RegHLInd, false),
	OpSubImmediateANoCarry:     subi(false),
	OpSubDirectAACarry:         sub8(RegA, true),
	OpSubDirectABCarry:         sub8(RegB, true),
	OpSubDirectACCarry:         sub8(RegC, true),
	OpSubDirectADCarry:         sub8(RegD, true),
	OpSubDirectAECarry:         sub8(RegE, true),
	OpSubDirectAHCarry:         sub8(RegH, true),
	OpSubDirectALCarry:         sub8(RegL, true),
	OpSubIndirectAHLCarry:      sub8(RegHLInd, true),
	OpSubImmediateACarry:       subi(true),
	OpCmpDirectAA:              cmpReg(RegA),
	OpCmpDirectAB:              cmpReg(RegB),
	OpCmpDirectAC:              cmpReg(RegC),
	OpCmpDirectAD:              cmpReg(RegD),
	OpCmpDirectAE:              cmpReg(RegE),
	OpCmpDirectAH:              cmpReg(RegH),
	OpCmpDirectAL:              cmpReg(RegL),
	OpCmpIndirectAHL:           cmpReg(RegHL),
	OpCmpImmediateA:            cmpi,
	OpAddDirectHLBC:            add16HL(RegBC),
	OpAddDirectHLDE:            add16HL(RegDE),
	OpAddDirectHLHL:            add16HL(RegHL),
	OpAddDirectHLSP:            add16HL(RegSP),
	OpAddImmediateSignedSP:     addSPsig,
	OpLoadOffsetHLSP:           loadHLStackOffset,
	OpCallNO:                   call(fNone),
	OpCallCA:                   call(fCarry),
	OpCallNC:                   call(fNotCarry),
	OpCallZE:                   call(fZero),
	OpCallNZ:                   call(fNotZero),
	OpReturnNO:                 ret(fNone),
	OpReturnCA:                 ret(fCarry),
	OpReturnNC:                 ret(fNotCarry),
	OpReturnZE:                 ret(fZero),
	OpReturnNZ:                 ret(fNotZero),
	OpRETI:                     reti,
	OpDecimalToBCD:             bcd,
...
func loadHLStackOffset(c *CPU) {
	orig := c.HL
	c.HL = Register(int32(c.SP) + int32(int8(nextu8(c))))
	c.SetFlags(Flags{
		Zero:      false,
		AddSub:    false,
		Carry:     c.HL < orig,
		HalfCarry: ((orig >> 8) & 0xf) > ((c.HL >> 8) & 0xf),
	})
	c.Cycles.Add(2, 12)
}
...
func _add(c *CPU, added uint8, useCarry bool) {
	oldval := c.AF.Left()
	carry := uint8(0)
...
func _sub(c *CPU, subbed uint8, useCarry bool) {
	oldval := c.AF.Left()
	carry := uint8(0)
	if useCarry && c.Flags().Carry {
		carry = 1
...
func bcd(c *CPU) {
	...
	if flags.AddSub {
		...
		if (acc&0xf) > 9 || flags.HalfCarry {
			acc += 0x06
		}
		if acc > 0x9f || flags.Carry {
			acc += 0x60
		}
	}

	flags.HalfCarry = false
	flags.Carry = orig > acc
	flags.Zero = acc == 0
...
func addSPsig(c *CPU) {
	orig := c.SP
	c.SP = Register(int32(orig) + int32(int8(nextu8(c))))
	c.SetFlags(Flags{
		Zero:      false,
		AddSub:    false,
		Carry:     c.SP < orig,
		HalfCarry: ((orig >> 8) & 0xf) > ((c.SP >> 8) & 0xf),
	})
	c.Cycles.Add(2, 16)
}
